...
func WalletReady(ctx context.Context) (bool, string) {
	// dcrd must be past initial block download. Only that refuses here: an
	// unreachable dcrd falls through to the wallet check, as it always has.
	if gate, _ := NodeWalletGateState(); gate == GateIBD {
	...
	status, err := FetchWalletStatus(ctx)
