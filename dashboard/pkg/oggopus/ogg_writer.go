...
func newOggWriter(out io.Writer) *oggWriter {
	return newOggWriterSerial(out, rand.Uint32())
}

// newOggWriterSerial is the seam tests use to pin the bitstream serial, the
// one value upstream draws at random.
func newOggWriterSerial(out io.Writer, serial uint32) *oggWriter {
	return &oggWriter{
		w:      out,
		serial: serial,
	}
